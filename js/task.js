...
const validateTask = (task) => task.trim() !== ''
...
const editOrDeleteTask = (i, isEdit) => {
    ...
    const existingListItem = taskList.querySelector('li:nth-child(' + (i + 1) + ')')
    ...
    if (isEdit) {
        ...
        const deleteButton = createButton('Usuń', () => editOrDeleteTask(i, false))
...
const createListItem = (taskName, i) => {
    ...
    const taskEditButton = createButton('Edytuj', () => editOrDeleteTask(i, true))
    const taskDeleteButton = createButton('Usuń', () => editOrDeleteTask(i, false))
